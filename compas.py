...
"""This function is loading the data, filter out samples from two dominant races: Caucasia or African-American, doing
one hot encoding, normalize the data, splitting the training and test set and Separate features, labels, and sensitive 
attribute. It mimic the whole data loading and processing pipeline of the original codebase """

def load_compas(path):
    data = pd.read_csv(path)
    data = data[data["race"].isin(["Caucasian", "African-American"])].copy()
    ...
    norm_cols = ["age", "juv_fel_count", "juv_misd_count",
    ...
    def split_xya(df):
        ...
        a = df["race_African-American"].to_numpy().astype(np.int64)
        return x, y, a
...
class SurrogateMLP(nn.Module):
    """Different architecture, different optimizer -- used as the source
    model for transfer attacks."""
...
def train_model(train_loader, input_size,
                ...
                lambda_fair=0.8, lambda_adv_fair=0.15,
                eps_train=0.05, max_epoch=60, lr=0.01, momentum=0.8,
                verbose=True, seed=0):
...
def main():
    ...
    parser.add_argument("--csv", required=True,
                        help="Path to compas-scores-two-years.csv")
    parser.add_argument("--epochs", type=int, default=60)
    parser.add_argument("--batch", type=int, default=256)
    parser.add_argument("--seed", type=int, default=0)
    parser.add_argument("--out", default="compas_results.png")
    parser.add_argument("--out-attacks", default="compas_attacks.png")
    ...
    (train_x, train_y, train_a), (test_x, test_y, test_a) = load_compas(args.csv)
